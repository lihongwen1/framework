...
class YuvToRgbConverter {
    ...
    fun yuvToNv21(image: Image): ByteArray {
        if (yuvBuffer == null) {
            ...
            val size = image.cropRect.width() * image.cropRect.height()
            yuvBuffer = ByteArray(size * pixelSizeBits / 8)
        }
        image.toByteArray(requireNotNull(yuvBuffer))
        return yuvBuffer!!
    }
    ...
    private fun Image.Plane.extractChromaInterleaved(
        ...
        if (outputOffset < outputArray.size) {
            ...
            outputArray[outputOffset] =
                uPlane.buffer.get((planeHeight - 1 + planeCrop.top) * rowStride + planeCrop.left * 2 + rowLength - 2)
        }
    }
